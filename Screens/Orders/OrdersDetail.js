...
export default function OrdersDetail({ navigation, route }) {
  ...
  const [defaultPrt, setDefaultPrt] = useState();
  ...
  const fireBaseRefInvoicesData = firebase
    .firestore()
    .collection("Invoice")
    .doc(route.params?.orderKeyN);
  useEffect(() => {
    fireBaseRefInvoicesData.get().then((res) => {
      if (res.exists) {
        const orderData = res.data();
        setKey(res.id);
        setAmount(orderData.Amount);
        setQuantity(orderData.Quantity);
        setDate(new Date(orderData.Date.toString()));
        setTotalPrice(orderData.TotalPrice);
        setValuePrt(orderData.PartyID);
        setValuePrd(orderData.ProductID);
  ...
  const onUpdatePress = () => {
    ...
    setIsLoading(true);
    const usersRef = firebase.firestore().collection("Invoice").doc(orderkey);
    usersRef
      .set({
        ProductID: valuePrd,
        PartyID: valuePrt,
      ...
      .then(() => {
        ...
        // setTotalPrice(orderData.TotalPrice);
        setIsLoading(false), navigation.navigate("OrdersList");
      })
      .catch((error) => {
        alert(error);
      });
  ...
  const onRegisterPress = () => {
    ...
    usersRef
      .add({
        ...
        ProductName: selectedProduct[0].label,
        PartyName: selectedParty[0].label,
        Potency: selectedParty[0].Potency,
        BatchNumber: selectedParty[0].BatchNumber,
        Quantity: quantity,
        Amount: amount,
        AddedOn: date,
        TotalPrice: totalPrice,
      ...
      {isLoading ? (
        ...
        <>
          ...
          >
            ...
            >
              <Text style={styles.buttonTitle}>Update Order</Text>
            </TouchableOpacity>
